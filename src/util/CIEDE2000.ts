 ...
 */
export const hue_d = (x: number, y: number): number => {
 ...
 */
export const deltaHue_d = ({ C1, C2, h1_d, h2_d }: HueHelper): number => {
  if (C1 * C2 === 0) {
    return 0;
  }
 ...
 * @param HueHelper param
 */
export const meanHue_d = ({ C1, C2, h1_d, h2_d }: HueHelper): number => {
  if (C1 * C2 === 0) {
    return h2_d + h1_d;
  }
